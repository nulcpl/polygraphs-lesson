# Introducing PolyGraphs: Running Philosophical Simulations and Analyzing Synthetic Data — Part 2: Analyzing Synthetic Data

**Brian Ball, David Freeborn, Federica Imbriale, Amil Mohanan and Nicolas Kuri Perez Villaman**

This (advanced) lesson in two parts sketches how to use the PolyGraphs Python package to (i) run philosophical simulations ([part 1](part-1-running-simulations.md)) and (ii) analyze the synthetic data it generates (part 2), so as to better understand social processes of knowledge production.
...
**The dataset is about 2 GB compressed and requires roughly 35 GB of free disk space when unpacked.** Check that you have this space available before you begin.

You can download the dataset for our case study on mis- and disinformation from its Zenodo record: [https://doi.org/10.5281/zenodo.22726906](https://doi.org/10.5281/zenodo.22726906). The archive is about 2 GB; after downloading, check that its MD5 checksum matches the one shown beside the file on the Zenodo record (the README of this repository shows the command for your operating system).

Once you have a local copy on your machine, you may want to move it (using the command line, or your graphic user interface) to 'polygraphs-cache\results', though you can extract and analyze it in any location provided that you use the relevant folder path below. Within the terminal, navigate to the folder containing the compressed dataset, and run:

```
tar -xzf cleaned_data_v3_pt.tar.gz
```

This creates the `cleaned_data_v3_pt` directory (within 'polygraphs-cache\results', if that is where you are working). Below, you will use the full path to its `unique_runs` subdirectory.
...
<p align="center"><img src="images/random_graph_sim_22700.png" alt="A collection of 64 blue dots ('nodes') densely connected by black lines ('edges')"></p>

*Figure 1. The random graph on which simulation 22700 in our dataframe was run.*
...
<p align="center"><img src="images/Average_Credences_sim_22700.png" alt="A visualization with a jagged blue line (called 'beliefs') plotting an upwards trajectory from about 0.5 to nearly 1.0 as the simulation step, or iteration, progresses from 0 beyond 4000."></p>

*Figure 2. The mean of nodes' credences at each recorded step in simulation 22700.*
...
Notice, however, that although this information can be worked out from data that we have, the Processor does not have a method for retrieving it. We can do this by defining a subclass of `Processor` that calculates network density and the number of reliable nodes. Run the complete definition below in a new notebook cell.[^4]
...
class DR_Processor(Processor):
    ...
    def density(self):
        # Use NetworkX to calculate the density of each graph
...
```
Now that we have defined this new class, we should put it to use. Set the `data_path` variable to point at the `unique_runs` subdirectory of the extracted `cleaned_data_v3_pt` directory:

```
data_path = '/full/path/to/cleaned_data_v3_pt/unique_runs'
```

Replace this with the actual path to the `unique_runs` directory created when you unpacked the dataset.
...
<p align="center"><img src="images/random_density_v_probability.png" alt="A visualization of Density (on the y axis) vs Random Probability (on the x axis). Density matches probability, plus or minus some noise."></p>

*Figure 3. A plot of density as a function of edge probability in random networks.*
...
<p align="center"><img src="images/er_rel_nodes_histogram.png" alt="A visualization of the number of simulations with each reliable node count from less than 35 to almost 60. There is a peak at 48, with over 400 sims having this reliable node count, with a gradual, and roughly symmetrical, drop in each direction along the x axis."></p>

*Figure 4. Histogram of reliable node count for size 64 networks with 0.75 reliability.*
...
A note of caution: `steps` records when a simulation ended, but a lower value does not necessarily mean that the community reached the truth more quickly. Simulations can also reach the wrong consensus, while runs that do not terminate earlier may reach the 100,000-step limit. Where the distinction matters, we should therefore consider whether the simulation successfully converged on the truth separately from how many steps it took.
...
<p align="center"><img src="images/random_steps_v_density.png" alt="A scatterplot of simulation steps for sims involving disinformation plus trust, with four discernible clusters of dots along the x axis (density): just above 0.1; above 0.2; just below 0.4; and near 0.5. On the y axis, steps range from 0 to 100,000: the lower density clusters have more dots nearer the top of the range."></p>

*Figure 5. A scatterplot of simulation steps vs density in simulations on size 64 random networks, with unreliable agents sharing disinformation, and all agents trusting what they are told.*
...
As this scatterplot reveals, sims with more reliable nodes in the third density (or probability) bucket typically have lower recorded step counts.
...
<p align="center"><img src="images/dt3_steps_v_rel_node_count.png" alt="A scatterplot of simulation steps (up to 100,000) vs reliable node count (c. 40 to 60) in disinfo_trust simulations on medium-high density size 64 random networks. There is a clear downwards trend in steps as reliable node count increases."></p>

*Figure 6. A scatterplot of simulation steps against reliable node count in the current collection of sims on medium-high density size 64 random networks.*
...
Seaborn allows us to plot data concerning ‘gullible’ agents side by side with data concerning more sceptical agents whose level of trust is ‘aligned’ to the level of reliability in the network (by setting the column of the plot, or ‘col’ variable, equal to the ‘op’ in a catplot). In both cases, simulations with more reliable nodes typically have lower recorded step counts – but, as is apparent, the more sceptical information processing strategy is associated with higher recorded step counts.

```
sns.catplot(data = dis2, x = "reliable_node_count", y = "steps", col='op')
```

<p align="center"><img src="images/dis2_steps_comparison.png" alt="Two scatterplots side by side: on the right (scepticism), the dots typically indicate higher numbers of steps than on the left (trust)."></p>

*Figure 7. A comparison of how reliable node count affects steps in a disinformative environment when agents are trusting (left) or sceptical (right).*
...
Amongst sceptical agents, simulations involving (‘binomial’) misinformation have lower recorded step counts than those involving (‘negative epsilon’) disinformation.

```
sns.catplot(data = scep4, x = "reliable_node_count", y = "steps", col='op')
```

<p align="center"><img src="images/scep4_steps_comparison.png" alt="Two more scatterplots side by side: on the left (misinformation), the dots indicate many fewer steps, especially at low reliable node counts, than on the right (disinformation)."></p>

*Figure 8. A comparison of how reliable node count affects steps amongst sceptical agents in the presence of misinformation (left) or disinformation (right).*
...
<p align="center"><img src="images/ba_1500_graph.png" alt="A visualization of a graph, with 32 blue dots (nodes) sparsely connected black lines (edges)."></p>

*Figure 9. Graph of the (32 node, 1 attachment) Barabasi-Albert network from sim 1500.*
...
<p align="center"><img src="images/density_v_attachments_ba64.png" alt="A scatterplot showing a single density dot for each number of attachments. Density increases with attachments, less so as number of attachments increases."></p>

*Figure 10. Density as a Function of Attachments in BA Networks of Size 64.*
...
<p align="center"><img src="images/ER_top4_betweenness_buckets.png" alt="Five collections of four boxplots (one for each op) showing that the number of steps increases as the mean betweenness centrality of the top 4 unreliable nodes increases - especially when unreliable nodes are disinformants."></p>

*Figure 12. Boxplots for each quartile of mean betweenness centrality for the top four unreliable nodes, for each operation in our dataset, on random graphs.*

Figure 13 shows a more surprising association: simulations with higher mean degree centrality among their four most highly connected unreliable agents tend to have lower recorded step counts. Could better-connected unreliable agents actually help a community reach the truth?

<p align="center"><img src="images/ER_top4_degree_buckets.png" alt="Five collections of four boxplots (one for each op) showing that recorded step counts tend to decrease as the mean degree centrality of the top 4 unreliable nodes increases, especially when unreliable nodes are disinformants."></p>
...
Up to this point, we have investigated the synthetic output data from PolyGraphs simulations, primarily from our case study of mis- and disinformation. In particular, we have: loaded the data, having refined the Processor to extract information about network structure, as well as the character of the informational environment; visualized the graphs on which simulations run, and the evolution of the agents' beliefs within a simulation over time (technically, iterations); obtained and plotted descriptive statistics; and more. These methods reveal patterns and suggest explanations. Statistical analysis can help us quantify uncertainty and assess whether associations persist after accounting for other variables. Statistical significance alone, however, does not establish that an association is causal.

If you are a quantitative social scientist, you will be aware that there are a great many different statistical methods, and that which ones are needed depends very much on the task at hand. In any case, we will not begin here to give you a systematic orientation to the variety of techniques that are available to you, both mathematically, and in Python; but we note that, when it comes to the efficiency of PolyGraphs simulations, one approach to statistical significance testing has been fully developed [here][1]. We hope this may be helpful in guiding your next steps, if this is a direction you wish to pursue.
